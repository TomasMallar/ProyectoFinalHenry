...
const Coments = () => {
    ...
    useEffect(() => {
        const getAllComents = async () => {
            ...
            const user = await axios.get(`/users/profile/${userId}`)
    ...
    const deleteComment = async (event) => {
        ...
        const responseUptaded = await axios.get(`/coments/updatedcomments/${id}` )
    ...
    const pagesHandler = async () => {
        if (cont + 1 <= Math.ceil(comments.length / 3)) {
            ...
            setCont(nextPage);
            //console.log(`/coments/${id}?page=${cont}`);
            const response = await axios.get(`/coments/${id}?page=${nextPage}`)
