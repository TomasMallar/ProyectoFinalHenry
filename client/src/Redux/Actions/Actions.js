...
export const getCarousel = () => {
  return async function (dispatch) {
    const response = await axios(
      `http://localhost:3001/products/advanced-search?name&category&type&orderBy&orderDirection&page=1&pageSize=20`
    );
    return dispatch({
...
export const getProductsAdvanceController = (
  name,
  category,
  type,
  orderBy,
  ...
  return async function (dispatch) {
    try {
      const response = await axios(
        `http://localhost:3001/products/advanced-search?name=${
          name || ''
        }&category=${category || ''}&type=${type || ''}&orderBy=${
          orderBy || ''
        }&orderDirection=${orderDirection || ''}&page=${page || 1}`
      );
...
export const PutProduct = (finalEditedProduct) => {
  return async function (dispatch) {
    try {
      await axios.put(
        `http://localhost:3001/products/update/${finalEditedProduct.id}`,
        finalEditedProduct
      );
  ...
  return async function (dispatch) {
    try {
      ...
      if (value === 'categories') {
        ...
        const responseTypeAndIngr = await axios(
          `http://localhost:3001/${value}/all`
        );
        return dispatch({
          type: GET_INGREDIENTS,
